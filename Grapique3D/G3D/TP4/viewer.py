...
class PhongMesh:
    ...
    def draw(self, projection, view, model, color_shader, phong_shader,
        ...
        GL.glUniformMatrix4fv(loc['projection'], 1, True, projection)
        GL.glUniformMatrix4fv(loc['model'], 1, True, model)
        GL.glUniform3fv(loc['light_direction'], 1, light_direction) #lumiere
...
def main():
    ...
    #
    cylinder = Cylinder(Kd=(0, 1, 0))
    base_shape = Node(transform=
                      translate(0.0, 2.0, 1.0)
                      @ rotate((1, 0, 0), 45)
                      @ scale(0.25, 2, 0.25))
    base_shape.add(cylinder)                    # shape of robot base
    limb_shape.add(base_shape)

    #
    cylinder = Cylinder(Kd=(0, 0, 1))
    arm_shape = Node(transform=translate(0.0, 1.0, 0.0) @ rotate((1, 0, 0), 25) @ scale(0.8, 0.8, 0.8))
